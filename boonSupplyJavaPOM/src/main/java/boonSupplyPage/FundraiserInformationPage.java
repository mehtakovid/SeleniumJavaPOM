...
public class FundraiserInformationPage extends FundraiserInformationPageElements {
	...
	public void EnterFundRaiserInfo(String firstname, String lastName, String email, String Phone) throws InterruptedException {
		...
		driver.findElement(NextButton).click();
		Thread.sleep(5000);
		if(driver.findElements(No_goalAtThisTime).isEmpty()) {
			action.moveToElement(driver.findElement(fundEmail)).click().build().perform();
			action.moveToElement(driver.findElement(NextButton)).click().build().perform();
		}else {
			System.out.println("App acted on First time email entry.");
		}
